...
// gemini-2.5-flash is current stable; set REACT_APP_GEMINI_MODEL to override (e.g. gemini-2.0-flash)
...
export const chatWithJsonTools = async (history, newMessage, executeFn, onProgress) => {
  ...
  for (let round = 0; round < 5; round++) {
    ...
    if (toolResult?._cardType === 'youtube_video') videoCards.push(toolResult);
